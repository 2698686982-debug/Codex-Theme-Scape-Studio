...
- Run the injector as a user LaunchAgent plist rather than an inferred `launchctl submit` job. Poll page targets and reinject after document loads; a debounced mutation observer plus a low-frequency safety timer handles in-page route changes.
- Keep a separate user LaunchAgent watchdog for ordinary Dock launches without CDP. Installation, start, and restore must remove the deprecated `com.openai.codex-dream-skin-studio.repair` job so an old submitted repair cannot enter a retry loop.
...
- Store script-managed mutable data under `~/Library/Application Support/CodexDreamSkinStudio`; keep the installed program under `~/.codex/codex-dream-skin-studio`. Renderer-uploaded raster themes are canvas-sanitized and stored in the Codex origin's IndexedDB.
