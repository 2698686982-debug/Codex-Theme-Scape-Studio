((cssText, themePack) => {
  ...
  const effectProfileForTheme = (theme) => {
    ...
    if (hue < 0) hue += 360;
    if (saturation < .20) return "silver-grain";
    if (theme?.layoutVariant === "terminal-grid") {
      if (hue >= 75 && hue < 165) return "matrix-rain";
      if (hue >= 180 && hue < 250) return "orbital-scan";
      return "synth-lasers";
    }
  ...
  const ensurePicker = () => {
    ...
    picker.innerHTML = `
      ...
      <section class="dream-skin-theme-panel" role="dialog" aria-label="Codex 主题中心" hidden>
        <header><span><b>Codex 主题中心</b><small>上传素材自动取色 · 系统默认 + 20 款内置主题</small></span><button type="button" aria-label="关闭主题中心">×</button></header>
        <div class="dream-skin-theme-scroll">
          ...
          <section class="dream-skin-theme-studio" aria-label="制作自定义主题">
            ...
            <p class="dream-skin-upload-status" role="status" data-tone="neutral">图片只保存在这台 Mac，不会覆盖 20 款内置主题。</p>
