...
/usr/bin/rsync -a \
  ...
  --exclude 'references/acceptance-report-*' \
  "$ROOT/" "$ENGINE/"

# Live QA screenshots stay in the source tree. They are intentionally omitted
# from the customer deployment ZIP to reduce size and avoid distributing local
# sidebar/task content; they are not required by the installer or runtime.
...
/usr/bin/printf '%s\n' \
  ...
  '' \
  '手动方式：双击“安装 Codex 主题编辑器.command”。安装完成后，桌面会出现启动、兼容导入、验证和恢复四个入口。' \
  '' \
  '右上角“主题”按钮最上方永久保留系统默认，另有 20 款内置主题，第 2 款是火影忍者，第 3 款是蓝白高达轨道指挥中心。可在主题中心直接上传图片、自动取色、选择布局并保存为额外“我的主题”。20 款内置主题分别拥有浮光、火花、雷电碎屑、气泡、花瓣、雨雪、烟尘、蒸汽、彩纸、数字雨或流星等唯一环境特效；素材原画保持固定，开启 macOS“减少动态效果”时会自动静止。' \
  '' \
